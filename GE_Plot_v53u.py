...
# Full build:
# - WGS84 shapefile DB with Edit / Replace / Delete
# - CRS: PRS92 Z3/Z4, Luzon1911 Z3/Z4, UTM 51N, WGS84
# - Input modes: A) Bearing&Distance PROMPTS (N corners => N+1 prompts; TP→C1 ... Cn→C1 close)
#                B) Input by Coordinates
# - Selection → Results (bearings/distances) + Map (WGS84)
# - Export: DXF (R2000), PDF, KML, KMZ
# - Safe read_db + "Reset shapefile (backup & recreate)"
...
def prj_epsg_or_default(base: str, default_epsg: int = STORAGE_EPSG) -> int:
    ...
    if os.path.exists(prj):
        try:
            c = CRS.from_wkt(open(prj, "r", encoding="utf-8").read())
            if c.to_epsg():
...
# SAFE reader: gracefully returns empty on missing/corrupt files
...
with st.sidebar:
    ...
    if st.button("🧹 Reset shapefile (backup & recreate)"):
        try:
            bkp = backup_corrupt_db(st.session_state["db_base"])
            reset_empty_db(st.session_state["db_base"])
            st.success(f"Reset done. Backup: {os.path.basename(bkp)}")
        except Exception as e:
...
with tab_input:
    ...
    # D) Grid + Edit/Replace/Delete
    st.markdown("---")
    st.header("Existing Lots (DataGrid) • Edit / Replace Geometry / Delete")
    df_all, shapes_all = read_db(st.session_state["db_base"])
    if df_all.empty:
        st.info("Shapefile empty. Add a lot above, or reset DB in sidebar if broken.")
    else:
        ...
        if "SEL_ATTR" not in st.session_state: update_selection_from_id()
            ...
            if st.button("Replace geometry now"):
                try:
                    if st.session_state["WORK_E"] is None or len(st.session_state["WORK_E"])<3:
                        ...
                        area, perim = polygon_area_perimeter(st.session_state["WORK_E"], st.session_state["WORK_N"])
                        df.loc[idx,"AREA"]=area; df.loc[idx,"PERIM"]=perim
                        df.loc[idx,"SRC_EPSG"]=int(CRS_OPTIONS[st.session_state["src_crs_name"]])
                        write_db(st.session_state["db_base"], df, shps)
...
with tab_results:
    ...
    if "SEL_LINES" in st.session_state and isinstance(st.session_state["SEL_LINES"], pd.DataFrame) and not st.session_state["SEL_LINES"].empty:
        ...
        st.info("Build a polygon or select a lot in the Input tab.")
...
<kml xmlns="http://www.opengis.net/kml/2.2"><Document>
  ...
  <Placemark>
    ...
    with open(filepath,"w",encoding="utf-8") as f: f.write(kml)
...
with tab_export:
    ...
    if "SEL_MAP_LON" in st.session_state and st.session_state["SEL_MAP_LON"] is not None:
        ...
        lon, lat = transform_coords(st.session_state["WORK_E"], st.session_state["WORK_N"],
                                    CRS_OPTIONS[st.session_state["src_crs_name"]], STORAGE_EPSG)
        attrs = {"LOT_NO":"WORK","CLAIMANT":"","AREA":0,"PERIM":0,"SRC_EPSG":CRS_OPTIONS[st.session_state["src_crs_name"]],
    ...
    if lon is None:
        ...
        with c2:
            if st.button("📄 PDF"):
                try:
                    ...
                    export_pdf(path, lon, lat, attrs or {})
                    st.success(f"Saved: {path}")
                except Exception as e:
        ...
        with c3:
            if st.button("🌍 KML"):
                try:
                    path=os.path.join(DB_DIR, f"{lot_slug}.kml")
                    export_kml(path, lon, lat, label, "")
                    st.success(f"Saved: {path}")
                except Exception as e:
        ...
        with c4:
            if st.button("🌐 KMZ"):
                try:
                    path=os.path.join(DB_DIR, f"{lot_slug}.kmz")
                    export_kmz(path, lon, lat, label, "")
                    st.success(f"Saved: {path}")
                except Exception as e:
