# ============================================================
# GE Plot v5.3l — Input • Results • Map Viewer • Export
...
with tab_input:
    ...
    if not os.path.exists(shp):
        ...
        if df.empty:
            ...
            sel_id = st.selectbox("Select record by ID", ids, index=0)
...
with tab_results:
    st.header("Results")
    try:
        st.write(f"**EPSG**: {ZONE_TO_EPSG.get(st.session_state.get('zone', DEFAULT_ZONE), 0)}  •  **Area**: {polygon_area(E_closed,N_closed):.3f} sq.m  •  **Perimeter**: {np.sum(np.hypot(np.diff(E_closed[:-1]), np.diff(N_closed[:-1]))):.3f} m")
        rows = lines_df.to_dict(orient="records")
        out_rows=[]
        for i,r in enumerate(rows, start=1):
            az=bearing_to_azimuth_deg(r["Bearing"])
            out_rows.append({"From":i,"To":i+1,"Bearing":azimuth_to_qeb(az),"Azimuth°":round(az,4),"Distance (m)":round(float(r["Distance"]),3)})
        st.dataframe(pd.DataFrame(out_rows), use_container_width=True, hide_index=True)

        fig, ax = plt.subplots()
        ax.plot(E_closed, N_closed, marker='o')
        ax.set_aspect('equal', adjustable='box')
        ax.set_title("Projected polygon (current CRS)")
        st.pyplot(fig)

        # NEW: show selected record lines (from DB), if any
        ...
        st.info("Enter inputs in the Input tab to see results here.")
